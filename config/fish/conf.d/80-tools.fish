...
set --query CARGO_HOME || set --local CARGO_HOME "$XDG_DATA_HOME"/cargo
if test -d "$CARGO_HOME"
    set --global --export CARGO_HOME "$CARGO_HOME"
...
set --query RUSTUP_HOME || set --local RUSTUP_HOME "$XDG_DATA_HOME"/rustup
if test -d "$RUSTUP_HOME"
    fish_add_path "$RUSTUP_HOME"/bin
    set --global --export RUSTUP_HOME "$RUSTUP_HOME"
end
